...
internal static class MathBlockProgramPopulationSearchGpuKernel
{
    ...
    internal static string CreateSource(string residentKernelSource)
    {
        ...
        AppendCatalog(source, ScalarGpuBlockCatalog.KernelSource, ScalarGpuBlockCatalog.KernelEntryPoint);
        AppendCatalog(source, VectorGpuBlockCatalog.KernelSource, VectorGpuBlockCatalog.KernelEntryPoint);
        AppendCatalog(source, ComplexGpuBlockCatalog.KernelSource, ComplexGpuBlockCatalog.KernelEntryPoint);
        AppendCatalog(source, MatrixGpuBlockCatalog.KernelSource, MatrixGpuBlockCatalog.KernelEntryPoint);
        AppendCatalog(source, ProbabilityGpuBlockCatalog.KernelSource, ProbabilityGpuBlockCatalog.KernelEntryPoint);
        AppendCatalog(source, SequencePathGpuBlockCatalog.KernelSource, SequencePathGpuBlockCatalog.KernelEntryPoint);
        AppendCatalog(source, StatisticsGpuBlockCatalog.KernelSource, StatisticsGpuBlockCatalog.KernelEntryPoint);
        AppendCatalog(source, GeometryGpuBlockCatalog.KernelSource, GeometryGpuBlockCatalog.KernelEntryPoint);
        AppendCatalog(source, GraphGpuBlockCatalog.KernelSource, GraphGpuBlockCatalog.KernelEntryPoint);
        AppendCatalog(source, AdvancedGpuBlockCatalog.KernelSource, AdvancedGpuBlockCatalog.KernelEntryPoint);
        AppendCatalog(source, TransportGpuBlockCatalog.KernelSource, TransportGpuBlockCatalog.KernelEntryPoint);
    ...
    private static void AppendCatalog(StringBuilder destination, string source, string entryPoint)
    {
        ...
        if (ReferenceEquals(transformed, source) || transformed.Contains(declaration, StringComparison.Ordinal))
            throw new InvalidOperationException($"GPU catalog entry '{entryPoint}' cannot become resident dispatch code.");
...
internal static class MathBlockProgramPopulationSearchResidentKernel
{
    internal const string Source = """
        ...
        extern "C" __global__ void mathblocks_program_population_search_begin(unsigned char* arena)
        {
            if (blockIdx.x != 0)
                return;
            if (mbp_read_int(arena, 0) != (int)0x4d425334 || mbp_read_int(arena, 4) != 10)
                return;
        ...
        extern "C" __global__ void mathblocks_program_population_search_prepare(unsigned char* arena)
        {
            ...
            mbp_ull enumeration_limit = mbp_read_ull(arena, 304);
            mbp_ull maximum_trials = mbp_read_ull(arena, 312);
            if (control_offset <= 0 || candidate_lane_count <= 0 ||

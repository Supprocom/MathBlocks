...
internal static class MathBlockProgramPopulationSearchResidentKernel
{
    internal const string Source = """
            ...
            int* cooperative_status)
        {
            ...
            for (int node = 0; node < operation_count; node++)
            {
                ...
                if (threadIdx.x == 0)
                {
                    ...
                    atomicAdd((unsigned long long*)(arena + control_offset + 24), 1ull);
                }
                __syncthreads();
            ...
            int* cooperative_status)
        {
            ...
            for (int node = 0; node < objective_node_count; node++)
            {
                ...
                if (kind == 5)
                {
                    if (threadIdx.x == 0)
                    {
                        int source = mbp_read_int(arena, descriptor + 24);
                        *cooperative_status = source >= 0 && source < node;
                        if (*cooperative_status != 0)
                            slots[node] = slots[source];
                    }
                    __syncthreads();
                    if (*cooperative_status == 0)
                        return false;
        ...
        extern "C" __global__ void mathblocks_program_population_search_setup(unsigned char* arena)
        {
            ...
            mbp_clear(arena + control_offset, 56);
            ...
            int lane_index)
        {
            ...
            int prepared_status = mbp_read_int(arena, candidate_entry);
            if (prepared_status != 1)
            {
                if (prepared_status == 7 && threadIdx.x == 0)
                    atomicAdd((unsigned long long*)(arena + control_offset + 40), 1ull);
                return;
            }
            if (threadIdx.x == 0)
                atomicAdd((unsigned long long*)(arena + control_offset + 32), 1ull);
            __syncthreads();
        ...
        extern "C" __global__ void mathblocks_program_population_search_commit(unsigned char* arena)
        {
            ...
            int candidate_lane_count = mbp_read_int(arena, 352);
            int catalog_offset = mbp_read_int(arena, 368);
            int catalog_authority_count = mbp_read_int(arena, 376);
            ...
            for (int ordinal = 0; ordinal < wave_result_count; ordinal++)
            {
                ...
                if (status == 7)
                {
                    mbp_ull proposal_cursor = mbp_read_ull(arena, candidate_entry + 40);
                    if (catalog_authority_count <= 0 || mbp_find_catalog_entry(
                            arena,
                            catalog_offset,
                            catalog_authority_count,
                            entry_size,
                            proposal_cursor) >= 0)
                    {
                        mbp_fail(arena, compact_offset, 4);
        ...
        extern "C" __global__ void mathblocks_program_population_search_finalize(unsigned char* arena)
        {
            ...
            mbp_write_ull(arena, compact_offset + 144, mbp_read_ull(arena, control_offset + 24));
            mbp_write_ull(arena, compact_offset + 152, mbp_read_ull(arena, control_offset + 32));
            mbp_write_ull(arena, compact_offset + 160, mbp_read_ull(arena, control_offset + 40));
            mbp_copy(

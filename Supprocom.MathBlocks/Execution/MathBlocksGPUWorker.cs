...
public sealed class MathBlocksGPUProgram : IDisposable
{
    ...
    internal static MathBlocksGPUProgram Create(
        MathBlockProgram program,
        IReadOnlyDictionary<string, MathBlockValue>? prototypeInputs)
    {
        ...
        try
        {
            ...
            foreach (var node in program.PlanNodes)
            {
                if (node.Kind != MathBlockProgramNodeKind.Operation)
                    continue;
                var kernel = MathBlocksGpuKernelModule.Resolve(node.OperationIdentity!);
            ...
            return new MathBlocksGPUProgram(
                program,
                slotOffsets,
                payloadOffsets,
                payloadCapacities,
    ...
    public void UploadInputs(IReadOnlyDictionary<string, MathBlockValue> inputs)
    {
        ...
        lock (stateLock)
        {
            ...
            if (executionInFlight)
                throw new InvalidOperationException("GPU input cannot change during execution.");
            foreach (var node in program.PlanNodes)
            {
    ...
    public void ExecuteResident()
    {
        lock (stateLock)
        {
            ThrowIfDisposed();
            if (!inputsUploaded)
                throw new InvalidOperationException("GPU inputs are not available.");
            MathBlocksCudaNative.EnsureContext();
            MathBlocksCudaNative.ThrowIfFailed(
    ...
    private static void ValidateProgram(MathBlockProgram program)
    {
        ...
        if (unsupportedKinds.Length != 0)
        {
            throw new NotSupportedException(
                $"The GPU program does not support: {string.Join(", ", unsupportedKinds)}.");
        }
        ...
        for (var nodeIndex = 0; nodeIndex < program.PlanNodes.Count; nodeIndex++)
        {
            ...
            if (!ContainsIdentity(MathBlocksGpuKernelModule.SupportedBlockIdentities, identity) &&
        ...
        if (missing.Length != 0)
            throw new NotSupportedException($"GPU implementations are missing: {string.Join(", ", missing)}.");
    }
        ...
        IReadOnlyDictionary<string, MathBlockValue>? prototypeInputs)
    {
        ...
        if (node.Kind == MathBlockProgramNodeKind.Constant)
            ...
            throw new InvalidOperationException($"GPU shape input for node {node.Index} is unavailable.");

        var scalar = value.AsScalar();
        if (scalar < 0d || scalar > int.MaxValue || scalar != Math.Truncate(scalar))
            throw new InvalidOperationException($"GPU shape input for node {node.Index} is not a nonnegative integer.");
        return (int)scalar;
    }
    ...
    private static unsafe int ResolvePayloadBytes(MathBlockValueKind kind, int capacity)
    {
        ...
        return kind switch
        {
            ...
            MathBlockValueKind.Graph => checked(capacity * sizeof(MathBlockGpuGraphEdge)),
            MathBlockValueKind.RunSet => checked(capacity * sizeof(MathBlockGpuRun)),
    ...
    {
        if (size < 0 || size > 20)
            throw new InvalidOperationException("GPU assignment shape is outside the operation domain.");
        ...
        bool valid)
    {
        ...
        *(MathBlockGpuSlot*)((byte*)arena + slotOffset) = slot;
    }
        ...
        MathBlockValue value)
    {
        ...
        if (count > capacity)
            throw new ArgumentException($"The GPU input requires {count} elements, but its capacity is {capacity}.");
        var slot = new MathBlockGpuSlot
        {
        ...
        {
            if (payloadOffset < 0)
                throw new InvalidOperationException("The GPU value has no payload allocation.");
            ...
            else if (value.Type.Kind == MathBlockValueKind.Graph)
            {
                ...
                var destination = (MathBlockGpuGraphEdge*)((byte*)arena + payloadOffset);
                for (var index = 0; index < count; index++)
                {
                    destination[index] = new MathBlockGpuGraphEdge
                    {
            ...
            else if (value.Type.Kind == MathBlockValueKind.RunSet)
            {
                ...
                var destination = (MathBlockGpuRun*)((byte*)arena + payloadOffset);
                for (var index = 0; index < count; index++)
                {
                    destination[index] = new MathBlockGpuRun
                    {
    ...
    private static unsafe MathBlockValue ReadValue(
        ...
        MathBlockType type)
    {
        var slot = *(MathBlockGpuSlot*)((byte*)arena + slotOffset);
        if (slot.Valid == 0)
            return MathBlockValue.Invalid(type, "The GPU result is invalid.");
        if (slot.Count < 0 || slot.Count > slot.Capacity)
            throw new InvalidOperationException("The GPU result count exceeds its arena capacity.");
        return type.Kind switch
        {
            ...
            MathBlockValueKind.RunSet => MathBlockValue.RunSet(
                ...
                type.Unit),
            _ => throw new InvalidOperationException($"Unsupported GPU output kind '{type.Kind}'.")
        };
    }
    ...
    private static unsafe MathBlockGraphEdge[] ReadGraphEdges(IntPtr arena, int payloadOffset, int count)
    {
        ...
        if (count == 0)
            return values;
        RequirePayload(payloadOffset);
        var source = (MathBlockGpuGraphEdge*)((byte*)arena + payloadOffset);
        for (var index = 0; index < count; index++)
    ...
    private static unsafe MathBlockRun[] ReadRuns(IntPtr arena, int payloadOffset, int count)
    {
        ...
        if (count == 0)
            return values;
        RequirePayload(payloadOffset);
        var source = (MathBlockGpuRun*)((byte*)arena + payloadOffset);
        for (var index = 0; index < count; index++)
    ...
    {
        if (payloadOffset < 0)
            throw new InvalidOperationException("The GPU result has no payload allocation.");
    }
    ...
    {
        if (disposed)
            throw new ObjectDisposedException(nameof(MathBlocksGPUProgram));
    }

    [StructLayout(LayoutKind.Sequential)]
    private struct MathBlockGpuSlot
    {
    ...
    [StructLayout(LayoutKind.Sequential)]
    private struct MathBlockGpuGraphEdge
    {
        public int From;
        public int To;
    ...
    [StructLayout(LayoutKind.Sequential)]
    private struct MathBlockGpuRun
    {
        public int Start;
        public int Length;
        public double Value;
...
internal static class MathBlocksGpuKernelModule
{
    ...
    {
        var feature = MathBlockGpuFeatureIndex.Resolve(identity);
        return feature.Family switch
        {
            MathBlockGpuFamily.Scalar => new KernelBinding(state.Value.ScalarFunction, feature.Opcode, 1),
            MathBlockGpuFamily.Vector => new KernelBinding(
                state.Value.VectorFunction, feature.Opcode, VectorGpuBlockCatalog.BlockSize),
            MathBlockGpuFamily.Complex => new KernelBinding(
                state.Value.ComplexFunction, feature.Opcode, ComplexGpuBlockCatalog.BlockSize),
            MathBlockGpuFamily.Matrix => new KernelBinding(
                state.Value.MatrixFunction, feature.Opcode, MatrixGpuBlockCatalog.BlockSize),
            MathBlockGpuFamily.Probability => new KernelBinding(
                state.Value.ProbabilityFunction, feature.Opcode, ProbabilityGpuBlockCatalog.BlockSize),
            MathBlockGpuFamily.SequencePath => new KernelBinding(
                state.Value.SequencePathFunction, feature.Opcode, SequencePathGpuBlockCatalog.BlockSize),
            MathBlockGpuFamily.Statistics => new KernelBinding(
                state.Value.StatisticsFunction, feature.Opcode, StatisticsGpuBlockCatalog.BlockSize),
            MathBlockGpuFamily.Geometry => new KernelBinding(
                state.Value.GeometryFunction, feature.Opcode, GeometryGpuBlockCatalog.BlockSize),
            MathBlockGpuFamily.Graph => new KernelBinding(
                state.Value.GraphFunction, feature.Opcode, GraphGpuBlockCatalog.BlockSize),
            MathBlockGpuFamily.Advanced => new KernelBinding(
                state.Value.AdvancedFunction, feature.Opcode, AdvancedGpuBlockCatalog.BlockSize),
            MathBlockGpuFamily.Transport => new KernelBinding(
                state.Value.TransportFunction, feature.Opcode, TransportGpuBlockCatalog.BlockSize),
            _ => throw new InvalidOperationException($"GPU family '{feature.Family}' is not supported.")
        };
    }

    private static ModuleState Load()
    {
        var source = ScalarGpuBlockCatalog.KernelSource + "\n" +
                     VectorGpuBlockCatalog.KernelSource + "\n" +
                     ComplexGpuBlockCatalog.KernelSource + "\n" +
                     MatrixGpuBlockCatalog.KernelSource + "\n" +
                     ProbabilityGpuBlockCatalog.KernelSource + "\n" +
                     SequencePathGpuBlockCatalog.KernelSource + "\n" +
                     StatisticsGpuBlockCatalog.KernelSource + "\n" +
                     GeometryGpuBlockCatalog.KernelSource + "\n" +
                     GraphGpuBlockCatalog.KernelSource + "\n" +
                     AdvancedGpuBlockCatalog.KernelSource + "\n" +
                     TransportGpuBlockCatalog.KernelSource;
